    ...
    try:
        ...
        sandbox = daytona.create(
            CreateSandboxFromSnapshotParams(
                snapshot=SNAPSHOT,
                language="python",
                name="zenith-browser-harness-one-sandbox",
                labels={LABEL: "zenith-zslr01"},
                public=False,
                ephemeral=True,
                ...
                env_vars={
                    ...
                    "BROWSER_USE_WORLDS_SEED": "411001",
        ...
        common_env = {
            ...
            "BROWSER_USE_WORLDS_HARBOR_COMMIT": "17bc7141ccb681e354e700fdf1dd90ee7c9856e3",
            "BROWSER_USE_WORLDS_HARBOR_VERSION": "0.20.0",
            "HARBOR_TRIAL_ID": "zenith-local-harness-smoke",
            "BROWSER_USE_WORLDS_CONTROL_DIR": "/tmp/customer-world/run",
            "BROWSER_USE_WORLDS_EVIDENCE_DIR": "/tmp/customer-world/evidence",
            "DATA_DIR": "/tmp/customer-world/data",
            "PORT": "3000",
...
""".replace("ENTRY", repr(entry))
        ...
        answer = ok(
            sandbox.process.exec("cat /tmp/outputs/final-answer.json", timeout=30), "answer"
        ).strip()
        ...
        outputs = []
        for name in ("final-answer.json", "page-info.txt", "final.png"):
            local = out / name
        ...
        (out / "result.json").write_text(json.dumps({
            ...
            "outputs": outputs,
            "deterministic_reward": int(bool(report.get("passed"))),
